...
import { RefreshCw, AlertTriangle, CheckCircle2, XCircle, Loader2, Users, MessageSquare, FolderSync } from 'lucide-react';
...
const statusSteps: { status: ResyncStatus; label: string; description: string; icon?: React.ReactNode }[] = [
  ...
  { status: 'syncing_contacts', label: 'Syncing Contacts', description: 'Syncing contacts from WhatsApp...', icon: <Users className="w-3 h-3" /> },
  { status: 'syncing_chats', label: 'Syncing Chats', description: 'Syncing conversations from WhatsApp...', icon: <FolderSync className="w-3 h-3" /> },
  { status: 'syncing_messages', label: 'Syncing Messages', description: 'Fetching messages from chats...', icon: <MessageSquare className="w-3 h-3" /> },
];
...
const getProgressPercent = (status: ResyncStatus, current?: number, total?: number): number => {
  ...
  // Base progress for this step
  const stepProgress = (index / statusSteps.length) * 100;
  // Progress within the step
  const stepSize = 100 / statusSteps.length;

  if (current && total && total > 0) {
    const withinStepProgress = (current / total) * stepSize;
    return Math.min(stepProgress + withinStepProgress, 95);
  }

  return Math.min(stepProgress + (stepSize * 0.5), 95);
};
...
export const ForceResyncModal: React.FC<ForceResyncModalProps> = ({
  ...
  const progressPercent = getProgressPercent(progress.status, progress.current, progress.total);
  const isInProgress = phase === 'progress' && !['idle', 'completed', 'error'].includes(progress.status);

  // Format detailed progress message
  const getDetailedMessage = (step: typeof statusSteps[0], prog: ResyncProgress): string => {
    if (step.status === prog.status && prog.current && prog.total) {
    ...
    >
      {phase === 'confirm' ? (
        ...
        <div className="flex flex-col">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 truncate">
                      {isActive && progress.subStep?.chatName
                        ? `Processing: ${progress.subStep.chatName}`
                        : step.description}
                    </p>
                  </div>
                </div>
          ...
          {/* Live Stats (shown during sync) */}
          {isInProgress && (progress.contactsSynced !== undefined || progress.conversationsSynced !== undefined || progress.messagesSynced !== undefined) && (
            <div className="grid grid-cols-3 gap-2 mb-6 p-3 bg-slate-700/30 rounded-lg">
          ...
          {progress.status === 'completed' && (
            <div className="grid grid-cols-2 gap-3 mb-6 p-4 bg-slate-700/50 rounded-lg">
              ...
              {(progress.messagesSynced ?? 0) > 0 && (
                <div className="text-center col-span-2">
                  <p className="text-2xl font-bold text-green-400">
                    {progress.messagesSynced ?? 0}
                  </p>
                  <p className="text-xs text-gray-400">Messages Synced</p>
                </div>
              )}
            </div>
          )}
